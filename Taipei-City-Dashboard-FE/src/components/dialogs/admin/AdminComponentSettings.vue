...
function handleToggleMapLayer(test) {
	...
	test[0].property = JSON.parse(propertyTemp);

	try {
		MapStore.addToMapLayerList(test);
	} catch (error) {
		dialogStore.showNotification("error", "渲染地圖:錯誤");
		console.error(error);
	}

	test[0].paint = paintTemp;
	test[0].property = propertyTemp;

	dialogStore.showNotification("info", "渲染地圖:預覽");
}
</script>
		...
		<div class="admincomponentsettings">
			...
			<div class="admincomponentsettings-tabs">
				...
				<button
					...
					@click="
						handleToggleMapLayer(currentComponent.map_config),
							(currentSettings = 'map')
					"
				>
					地圖
						...
						<button
							@click="
								handleToggleMapLayer(
									currentComponent.map_config
								)
							"
							class="map-preview-container-button"
						>
							測試渲染
						</button>
						<!-- 
						<MapContainer
							:config="currentComponent.map_config[0]"
							@toggle-layer="handleToggleMapLayer"
						/> -->
					</div>
				</div>
			</div>
...
.map-preview-container {
	height: 40vh; // Set height according to your UI design
	width: auto; // Set width according to your UI design
	overflow: visible;
}

.map-preview-container-button {
	position: absolute;
	bottom: 0;
	right: 0;
	padding: 4px 8px;
